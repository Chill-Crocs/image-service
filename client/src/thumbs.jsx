        ...
        <div className="image-modalCol2">
          <div className="image-modalThumbBox">
            ...
            <div className="image-thumbCouple">
              {subArray.map((image, index) => <div name={index + imageArray.length} role="button" tabIndex={0} onClick={this.thumbClick} onKeyUp={this.thumbClick} key={image + (Math.random() * Math.floor(100))}><img name={index + imageArray.length} className={thumbClass[0]} alt="noimage" src={image} /></div>)}
            </div>
          </div>
        </div>
      );
